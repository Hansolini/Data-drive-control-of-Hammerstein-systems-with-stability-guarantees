%
%              C = VRFT1_ry(u,y,Mr,B,W,k,preFilt)
%     [C, theta] = VRFT1_ry(u,y,Mr,B,W,k,preFilt)
% 
...
%     of an f(k,k) model used to make an approximate model of the plant. Otherwise 
%     this parameter must be empty: [].
% preFilt: if this parameter is set to 'n', the prefilter is disabled (the 
%     prefilter L(z) is set to 1). If this parameter is empty ([]) the function uses 
...
function [C, theta_vector] = VRFT1_ry(u,y,Mr,B,W,k,preFilt)
...
if( isempty(preFilt) )
    ...
    L = minreal( minreal((1 - Mr)*Mr)*minreal(W*(U\1)) ); %inv(U)<->(U\1) %calcolo il filtro ottimo
else
    try
        if( preFilt == 'n')
...
[Cr, Cy, An, Fn, thetaR, thetaY] = vrft_engine(u,y1,y2,M,[],B,[],L,[],k,'n');
C = Cr;
theta_vector=thetaR;
