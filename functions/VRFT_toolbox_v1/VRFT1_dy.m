%
%              C = VRFT1_dy(u,y,Md,B,W,k,preFilt)
%     [C, theta] = VRFT1_dy(u,y,Md,B,W,k,preFilt) 
%
...
% preFilt: if this parameter is set to 'n', prefiltering is disabled (the 
...
function [C, theta] = VRFT1_dy(u,y,Md,B,W,k,preFilt)
...
if( isempty(preFilt) )
    ...
    L = minreal( minreal((1 - Md)*Md)*minreal(W*(U\1)) );  %calcolo il filtro ottimo
else
    try
        if( preFilt == 'n')
...
[Cr, Cy, An, Fn, thetaR, thetaY] = vrft_engine(u,y1,y2,[],Md,[],B,[],L,k,'n');
C = Cy;
theta=thetaR;
